...
impl Roameo {
    ...
    pub fn has_essid(&self, interface: &str, essid: &str) -> Result<(), io::Error> {
        ...
        unsafe {
            if ioctl(socket.as_raw_fd(), SIOCGIWESSID, &req) == -1 {
                info!("ioctl call for {} failed with: {}",
                      interface, io::Error::last_os_error());
        ...
        debug!("Interface {} connected to {:?} ({:?})", interface, id, essid);
        if id == essid {
